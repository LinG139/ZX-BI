...
public class ClaudeClient implements AiClient {
    ...
    public String doChatWithHistory(String message, boolean isChartAnalysis, String customPrompt, List<ZhiPuClient.Message> history) {
        try {
            if (config == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "Claude配置未初始化");
            }
            ...
            String systemPrompt = isChartAnalysis ? CHART_ANALYSIS_SYSTEM_PROMPT : 
                    (customPrompt != null && !customPrompt.trim().isEmpty() ? customPrompt : "");
            if (!systemPrompt.isEmpty()) {
                requestBody.put("system", systemPrompt);
            }

            String jsonBody = JSONUtil.toJsonStr(requestBody);
            log.debug("Claude请求体: {}", jsonBody);
            ...
            if (status != 200) {
                log.error("Claude调用失败，HTTP状态码: {}, 响应: {}", status, response.body());
            ...
            if (jsonResponse.containsKey("error")) {
                ...
                log.error("Claude返回错误: {}", errorMessage);
            ...
            if (contentArray == null || contentArray.isEmpty()) {
                log.error("Claude响应中没有content，响应: {}", responseBody);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI 响应异常");
            }

            JSONObject firstContent = contentArray.getJSONObject(0);
            String content = firstContent.getStr("text");
            if (content == null || content.trim().isEmpty()) {
                log.error("Claude返回的内容为空，响应: {}", responseBody);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI 返回内容为空");
            }

            log.info("Claude调用成功，返回内容长度: {}", content.length());
            return content;

        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            log.error("Claude调用过程中发生未预期异常", e);
