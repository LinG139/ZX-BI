...
/**
 * OpenAI客户端（兼容OpenAI API格式的通用客户端）
 */
@Service
@Slf4j
public class OpenAiClient implements AiClient {
    ...
    public void setConfig(AiConfig config) {
        this.config = config;
        log.info("OpenAI兼容配置已更新: platformName={}, chatModel={}, chartModel={}, baseUrl={}",
    ...
    public String doChatWithHistory(String message, boolean isChartAnalysis, String customPrompt, List<ZhiPuClient.Message> history) {
        try {
            if (config == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "OpenAI配置未初始化");
            }
            ...
            // 如果baseUrl已经包含完整路径，直接使用；否则拼接chat/completions
            String url;
            ...
            log.info("调用OpenAI兼容接口，模型: {}, 类型: {}, 消息长度: {}, 历史消息数: {}, URL: {}",
                    modelId, isChartAnalysis ? "图表分析" : "普通聊天", message.length(), history.size(), url);
            ...
            if (status != 200) {
                log.error("OpenAI调用失败，HTTP状态码: {}, 响应: {}", status, response.body());
            ...
            if (messageObj == null) {
                log.error("OpenAI响应中没有message字段，响应: {}", responseBody);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI 响应异常");
            }

            String content = messageObj.getStr("content");
            if (content == null || content.trim().isEmpty()) {
                log.error("OpenAI返回的内容为空，响应: {}", responseBody);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI 返回内容为空");
            }

            log.info("OpenAI调用成功，返回内容长度: {}", content.length());
            return content;

        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            log.error("OpenAI调用过程中发生未预期异常", e);
