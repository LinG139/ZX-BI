...
public class DeepSeekClient implements AiClient {
    ...
    public void setConfig(AiConfig config) {
        this.config = config;
        log.info("DeepSeek配置已更新: platformName={}, chatModel={}, chartModel={}, baseUrl={}",
    ...
    public String doChatWithHistory(String message, boolean isChartAnalysis, String customPrompt, List<ZhiPuClient.Message> history) {
        try {
            if (config == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "DeepSeek配置未初始化");
            }
            ...
            log.info("调用DeepSeek，模型: {}, 类型: {}, 消息长度: {}, 历史消息数: {}, URL: {}",
            ...
            log.debug("DeepSeek请求体: {}", jsonBody);
            ...
            if (status != 200) {
                log.error("DeepSeek调用失败，HTTP状态码: {}, 响应: {}", status, response.body());
            ...
            if (messageObj == null) {
                log.error("DeepSeek响应中没有message字段，响应: {}", responseBody);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI 响应异常");
            }

            String content = messageObj.getStr("content");
            if (content == null || content.trim().isEmpty()) {
                log.error("DeepSeek返回的内容为空，响应: {}", responseBody);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI 返回内容为空");
            }

            log.info("DeepSeek调用成功，返回内容长度: {}", content.length());
            return content;

        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            log.error("DeepSeek调用过程中发生未预期异常", e);
